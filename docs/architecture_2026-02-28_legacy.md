...
Адрес: https://forecasteconomy.com
Сервер: Timeweb Cloud, Ubuntu 24.04, 1 CPU / 2 GB RAM / 30 GB NVMe, Москва
DNS: nic.ru → A-записи `@` и `www` → 5.129.204.194
...
Пользователь (браузер)
        ...
        ▼
┌──────────────────┐
│  Caddy (systemd) │  ← автоматический HTTPS, редирект HTTP→HTTPS
│  :80 / :443      │
│  reverse_proxy    │
│  localhost:3000   │
└────────┬─────────┘
...
| Nginx Alpine | Раздача SPA + reverse proxy к API (внутри Docker) |
| Russian Trusted CA | SSL-сертификат для HTTPS к Росстату |
| nic.ru | DNS-хостинг (A-записи → Timeweb) |
...
```
Браузер → GET https://forecasteconomy.com/api/v1/indicators/cpi/inflation
    │
    ▼
Caddy (:443, TLS termination)
    │ reverse_proxy localhost:3000
    ▼
Nginx (frontend контейнер, :3000)
...
| Сервер перезагрузился | Docker restart: unless-stopped + Caddy systemd enabled → всё поднимается |
| SSL-сертификат истекает | Caddy автоматически обновляет Let's Encrypt сертификат (каждые 60 дней) |
...
| Компонент | Детали |
|---|---|
| Домен | forecasteconomy.com |
| DNS-хостинг | nic.ru |
| A-записи | `@` → 5.129.204.194, `www` → 5.129.204.194 |
| TLS | Caddy + Let's Encrypt (автоматическое обновление) |
| Порты | Caddy: 80/443, Frontend (Docker): 3000, Backend: 8000 |
| Конфиг Caddy | `/etc/caddy/Caddyfile` |
| HTTP→HTTPS | Автоматический редирект через Caddy |
