...
**World extension last verified:** 2026-08-22 (IMF WEO `NGDPD`/`NGDPDPC` via SDMX 3.0;
...
> **Single source of truth** для актуальных URL/файлов, откуда тянется каждый из 116 source-индикаторов. Если меняется источник в коде парсера или `seed_data.py` — **обязательно** актуализировать этот файл (см. [`AGENTS.md::Шаг 4`](../AGENTS.md#шаг-4--протокол-актуализации-документации-критично)).
>
...
| `imf` | International Monetary Fund, World Economic Outlook | SDMX 3.0 dataflow `IMF.RES/WEO` (latest), без ключа: `https://api.imf.org/external/sdmx/3.0` | Ряды `NGDPD` (ВВП в текущих $, млрд) и `NGDPDPC` (ВВП на душу, $). Ключ COUNTRY.INDICATOR.FREQUENCY, например `USA.NGDPD.A`. Публичный `source_url`: `https://www.imf.org/en/Publications/WEO`. Ingest: `app.services.world_imf_ingest.run_imf_weo_ingest`. Россия в рейтинг — служебные unlisted ряды `weo-gdp-usd` / `weo-gdp-per-capita-usd` (`parser_type=imf_weo`), не `WorldCountry` RU и не рубль×курс. WEO-оценка текущего года — наблюдение; годы > текущего календарного (среднесрочный outlook) не пишем; в `world_forecasts` не пишем. |
