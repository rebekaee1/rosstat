...
**Last updated:** 2026-09-25 (cutover деплоя: preboot backend, Caddy на `127.0.0.1:3000`, FLUSHDB после watch; cgroup backend 2G и Redis 384mb).

**Previous:** 2026-09-20 (локальный пакет `fix/history-access-reliability`: история, per-IP доступ, архив ассетов и происхождение метрик Пульса; прод не принят).
...
**История SEO, решение владельца 2026-09-20.** Все действующие публичные canonical-страницы должны входить в sitemap. Curated-отбор мировых годов, повышенный порог точек годов РФ, порог стран мирового рейтинга и выборка популярных/соседних пар сняты. Условия самого SSR (существующие данные, допустимая территория, сопоставимость), канон и исключение служебных URL остаются; sitemap не создаёт страницы, для которых нет маршрута. Региональный реестр учитывает также округ/РФ, скрытые из каталога публичные ряды и месячные карточки. Генерация потоковая, границы/счётчики версионированы `public-v3`. Детали и статус выкладки — [CONTEXT](../CONTEXT.md#history-access-reliability-2026-09-20) и [backlog](backlog.md#history-access-reliability-2026-09-20).
...
- **Deploy 502 / cutover (2026-09-25)** — единственный слушатель `127.0.0.1:3000` нельзя снимать на время entrypoint backend: Caddy в этот момент отдаёт 502. `scripts/deploy.sh` сначала прогоняет миграции и seed во временном контейнере (он не занимает DNS-имя `backend`), затем коротко пересоздаёт backend и только после `/health/ready` меняет frontend. Caddy проксирует `127.0.0.1:3000`, не `localhost` (иначе часто `[::1]`). FLUSHDB кэша — после 15-минутного watch, чтобы холодный SSR не валил TTFB-откат; до flush старый HTML ссылается на hashed-ассеты из архива. Оставшееся окно 502 — повторный старт uvicorn, пока nginx проксирует на ещё не слушающий `backend`. `/assets/behavior-standalone.js` без хэша отдаётся `no-cache`, как `/consent.js`.
...
- **Health endpoints** — `/api/v1/health/live` (тривиальный liveness), `/api/v1/health/ready` (readiness: SELECT 1 + PING обоих Redis + `scheduler.running`; устаревший ETL и fd >80% soft ulimit — мягкая `degraded: true`, не 503 — см. Р-8), `/api/v1/analytics/health` (failed analytics syncs за 24ч). Docker-healthcheck и deploy-smoke смотрят на `/health/ready`; external uptime monitor — на `/health/live` + `/health/ready`. Поле `checks.open_fds` — текущее число дескрипторов процесса.
- **FD gauge / ulimit (2026-09-20)** — `/api/v1/metrics` отдаёт `fe_process_open_fds` и `fe_process_open_fds_limit`. Сервис `backend` в `docker-compose.yml` поднимает `ulimits.nofile` soft/hard 65536 (Docker-дефолт 1024 на python-slim исчерпывался плановыми Session/шрифтами). Staleness-алерт дописывает `fd N/limit`.
