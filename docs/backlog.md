...
| Future (отложено) | F1 крипто (закрыто: BTC/ETH/SOL), F2 регионы, **F3 Telegram-бот** (подписка на indicator, daily push, custom alerts), **F4 Embed-виджеты UI** (дизайн + копи-кнопка кода + CSP), **F5 Календарь публикаций UI** (backend готов 1208 events), **F6 Продажи/регистрация автомобилей — ЗАБЛОКИРОВАНО источником** (см. ниже) — см. раздел Roadmap. |
