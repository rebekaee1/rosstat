...
3. **IndexNow full-site** (`indexnow.ping_full_site` + `backend/scripts/indexnow-ping-all.py`): приоритетные секции из `site_urls.section_names_static` через Redis-очередь (батчи ~800, debounce 24ч/URL, per-host). Слепой проход всех URL (включая региональные годы) не делается — длинный хвост идёт очередью переобхода Вебмастера. Разовый исторический прогон 2026-07-04 закрыл тогдашний реестр; после dual-host ETL пингует затронутые карточки на обоих хостах.
