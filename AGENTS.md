...
**Last updated:** 2026-09-20 — локальная реализация `fix/history-access-reliability`, ещё не приёмка прода. Возраст истории сам по себе не даёт `noindex`; мировые годы сохраняют curated-гейт. Доступ не блокируется по `fe_bind`, nginx ограничивает каждый IP без UA-обхода. Архив старых и новых ассетов публикуется до нового HTML, очистка — после watch; Пульс разделяет UTM, платное привлечение, доход РСЯ и отсутствие данных. Инварианты — [CONTEXT](CONTEXT.md#history-access-reliability-2026-09-20) и [enterprise_resilience](docs/enterprise_resilience.md); статус — [backlog](docs/backlog.md#history-access-reliability-2026-09-20). Деплой только до явно одобренного SHA с проверкой всего диапазона миграций — [workflow](docs/workflow.md#прод-деплой).
