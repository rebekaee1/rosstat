...
> **Хроника в этом файле — не живые счётчики.** Числа URL и индексации
> брать из `/sitemap-stats.json` и `docs/site-inventory.json`. Не цитировать
> «~43,3k URL» из истории как текущий факт.
...
**Last updated:** 2026-09-20 — локальная реализация `fix/history-access-reliability`, ещё не приёмка прода. Решение владельца 2026-09-20: все действующие публичные canonical-страницы входят в sitemap; возраст, curated dataset, популярность и размер среза не служат отдельными фильтрами. Условия существования SSR и исключение редиректов сохраняются. Доступ не блокируется по `fe_bind`, nginx ограничивает каждый IP без UA-обхода. Архив старых и новых ассетов публикуется до нового HTML, очистка — после watch; Пульс разделяет UTM, платное привлечение, доход РСЯ и отсутствие данных. Инварианты — [CONTEXT](CONTEXT.md#history-access-reliability-2026-09-20) и [enterprise_resilience](docs/enterprise_resilience.md); статус — [backlog](docs/backlog.md#history-access-reliability-2026-09-20). Деплой только до явно одобренного SHA с проверкой всего диапазона миграций — [workflow](docs/workflow.md#прод-деплой).

**Previous:** 2026-07-06, поздний вечер (**CTO-аудит: дозакрытие хвостов В/П/О/Н** — правдивость (московское «сегодня» `display.today_msk()`, adjacency-guard кросс-каденс QoQ/MoM, честные lastmod/stale-тикер/нейтральные рейтинги, юниты 79 региональных показателей), перфоманс (батч-hero каталога одним запросом, композитный индекс `region_data`, сравнение регионов одним запросом, **nginx rate-limit SSR/OG для не-поисковых UA** — trap про capture-группы в nginx map — `CONTEXT.md::traps`), ops (offsite-скаффолд + Redis-heartbeat pg-backup → `/health/ready`, региональный сидер стрим+COPY ≈30с, Postgres 1G, конфигурируемый пул БД с посчитанным бюджетом); матрица покрытия всего плана — `docs/backlog.md::2026-07-06`. Ранее тем же вечером **CTO-аудит: Волны 0–5, ветка `analysis/project-review-2026-07-06`** — сквозной аудит и доводка всего проекта за один проход; полная запись — `docs/backlog.md::2026-07-06 — CTO-аудит`. Что изменилось системно и где теперь искать: **(1) Правдивость витрины** — единая точка форматирования публичных значений `backend/app/services/display.py` (display_value/format_number_ru/annual_summary; русская десятичная запятая везде: SSR, OG, RSS, embed, calc/регионы на фронте) — при выводе значения пользователю ходить через неё, не изобретать per-case; **(2) Наблюдаемость** — реальный readiness `/api/v1/health/ready` (БД + оба Redis + планировщик; на него смотрят docker-healthcheck и deploy-smoke), APScheduler error/missed-listener → Telegram, staleness-монитор, 5xx/fail-open счётчики в `/metrics`; **(3) Write-контур** — derived-пересчёт инкрементальный по dependency-графу (П-2, полный прогон только `scripts/rebuild-all-derived.py`), batch upsert, seed-hash skip (быстрый старт), SSR HTML/sitemap/OG кэши; **инвалидация кэша — версионированные namespace** `fe:{ns}:v{N}` + `bump_namespaces()` (один INCR, БЕЗ SCAN; ключи строить только через `core/cache.py::versioned_key`) — деплой этой ветки = разовый холодный кэш, делать FLUSHDB DB 0 и деплоить в низкий трафик; **(4) Тесты** — route-smoke, alembic на чистой БД в CI (`scripts/check-migration-drift.py`), SSR-матрица, parser-fixtures, forecast-снапшоты, компонентные, Playwright E2E (`scripts/e2e/smoke.mjs`, job `e2e` в CI); **(5) Ops** — прод-деплой = `scripts/deploy.sh` (preflight-бэкап, ff-only guard, SHA-образы, расширенный smoke, автооткат), выделенный `redis-state` (noeviction+AOF) для сессий, распределённые локи джобов (state-Redis SET NX), prod-assertions при debug=false, body-limit/таймауты nginx+Caddy; **(6) SEO** — 301-карта легаси-URL `app/data/legacy_redirects.py` (легаси-коды, unlisted-siblings, регион-префиксы; SSR отдаёт 301 до рендера), блок годовых ссылок на карточке, еженедельный отчёт индексации Вебмастера в Telegram (`webmaster_indexing_report.py`, пн 09:30); **(7) Гигиена** — pypdf (PyPDF2 удалён), транзитивный lock `backend/constraints.txt` (регенерировать после правки requirements), `.env.example` = полный каталог compose-переменных, guard дрейфа доков `scripts/audit-doc-counters.py` в `check-all.sh` — счётчики в README/CONTEXT/data_sources теперь проверяются автоматически, править цифры руками без прогона нельзя.) Ранее тем же днём (**«Аналитика 2.0»: идентичность, цели, executive-BI, OLAP** — новый ADR-0010: постоянный `visitor_id` (localStorage, хэш в events/sessions) + граф идентичности `identity_links` + ретро-мост `_ym_uid`→`raw_metrika_visits` тем же `stable_hash`; гео по IP своими силами (DB-IP Lite mmdb, `services/geoip.py`, автообновление ежемесячно, IP не храним); портрет сессии расширен (connection/deviceMemory/cores/тема/ориентация/webdriver) + полная атрибуция (utm_term/content/yclid + серверный канал `traffic_channel.py`); web-vitals v4 + `js_error` + `api_timing` + активное время в dwell + `block_view` по `data-block` на всех шаблонах SPA + **standalone-сборка behavior.js для чистых SSR-страниц** (Vite-entry `behavior-standalone`, строка в `_SSR_CHROME_FOOTER` — ~43k SEO-URL больше не слепая зона); вычислительный фундамент: серверная сессионизация 30 мин → `server_sessions`, таксономия целей `goal_taxonomy.py` (tier + вес), словарь `metrika_goals` из Management API (числа goals_json расшифрованы), rollup'ы `daily_traffic`/`daily_goals`/`daily_pages` (15-мин инкремент + ночной хвост 60 дней, `tasks/analytics_rollups.py`); единый слой витрин `services/analytics_marts.py` — BI, Пульс (`snap["marts"]` = полный дневной контекст) и бот читают одни функции; BI перестроен в 10 разделов: Дерево (North Star + 4 драйвера + таргеты `data/bi_targets.py` + calendar-heatmap) · Привлечение (+сегменты канал×устройство×новизна, +каркас денег Директа `direct_costs`) · Аудитория (+гео, +«Люди» со скорингом) · Поведение (ECharts sankey/sunburst + блочное внимание) · Конверсия (собственная realtime-воронка + истинная Метрика-воронка по бизнес-целям) · Удержание · Спрос и SEO · Что менять (квадранты×adoption×copy) · Срезы (ClickHouse OLAP: compose-сервис, 15-мин курсорный синк `clickhouse_sync.py`, конструктор метрика×измерения с белым списком, деградация без влияния на сайт) · Надёжность (vitals p75/ошибки/латентность API/полнота сбора); realtime-алерты аномалий `analytics_alerts.py` (обвал трафика/всплеск ошибок/тишина сбора/лаги, антиспам 2 ч); Alembic `20260706_analytics2`; тесты `test_analytics2.py`. Детали — ADR-0010, `docs/backlog.md`.) Ранее 2026-07-05, ночь-5 (**собственный слой аудитории + истинность цифр BI**, ADR-0009 пп. 7–8: behavior.js шлёт `session_start` (UA/экран/язык/таймзона/referrer/UTM) → таблица `behavior_sessions` + UA-парсер `services/ua_parser.py` без зависимостей; BI-вкладка «Аудитория» — наши срезы устройств/браузеров/ОС/экранов/языков РЯДОМ с референсом Метрики (сверка двух счётчиков); канон `admin_bi._has_goals()` — goals_json Logs API это `{"goals": "[…]"}` у КАЖДОГО визита, truthy-чек давал ложную конверсию 91–100% (реально ~26%), инвариант «цель ⊆ вовлечённые»; retention переведён на дневные когорты (недельные при истории <2 недель — «0%» по определению); пульс недели 7×24 — два НЕ суммируемых слоя (визиты Метрики + просмотры собственного счётчика); календарь KPI дозаполняется нулями. Детали — `docs/backlog.md` «ночь-5».) Ранее той же ночью (**аудит региональной БД против Excel-первоисточника + доводка калькуляторов**: перепарсинг всех 461 листа Pril_Region_Pokaz_2025 → 909 701 точка совпадает с артефактом и Postgres байт-в-байт, 0 расхождений (51 225 остальных точек — задокументированный дособор 2022/2023/Word, ADR-0008); калькуляторы ипотеки/сложных процентов: единый `CalcSlider`, ось Y через `formatCompactTick`+`compactTickAxisWidth`, «г.» на оси X убрано; навбар: «Главная» убрана с десктопа (наезжала на логотип на 1024px), «О проекте» скрыт до xl. Детали — `docs/backlog.md`.) Ранее тем же днём (**фикс BI-входа + самообновление версии у посетителей + «Калькуляторы»**: `/auth/login` теперь отдаёт `is_admin` как и `/auth/me` (общий `_serialize_with_admin`) — без него админ после входа на `/admin/bi` видел «404» до перезагрузки; BI получил период «Год»; `main.jsx` слушает `vite:preloadError` → одноразовый reload при деплое посреди сессии (ChunkLoadError у сидящих на сайте — «поехавшая» вёрстка в Вебвизоре была реплеем старых записей против новых asset-hash); навбар: порог непрозрачности 200px→24px + фолбэк `@supports not (backdrop-filter)` (наложение на скрине руководителя); меню-пункт «Калькуляторы» (dropdown): инфляция + новые `/calculator/mortgage` (аннуитет) и `/calculator/compound` (сложные проценты) — общий `lib/calcFormat.js`, PAGE_META + nginx + STATIC_PAGES; Детали — `docs/backlog.md`. Ранее в тот же день: **Admin-BI `/admin/bi`** — служебный дашборд полной статистики (KPI по дням / привлечение / воронка «канал → вовлечение → цель» / retention-когорты / качество страниц dwell×dead×bounce / спрос vs покрытие / все внутренние поиски / граф навигации / события гость-зарег / гипотезы Пульса / инвентаризация датасета), backend `services/admin_bi.py::build_bi_dashboard` + `api/admin_bi.py`, доступ по `settings.admin_emails` (RUSTATS_ADMIN_EMAILS; не-админ → 404, `/auth/me` отдаёт `is_admin` только админам, ссылка в кабинете), самообновление: Redis-кэш 15 мин + refetchInterval 15 мин, идемпотентный `backend/scripts/create-admin.py`, тесты `test_admin_bi.py`; **сбор ВСЕХ внутренних поисков** — единый хук `frontend/src/lib/useSearchTracking.js` → событие `search_query{q,results,context}` (контексты: map-metric / regions-list / region-profile / compare-region / compare-region-indicator; глобальный ⌘K, compare_search, table_search уже собирались) — автоматически попадает в Пульс/дайджест/BI; **фикс поиска карты «Свой показатель»** — dropdown открывался только по onFocus (после «сбросить» фокус жив → ввод не отображался), теперь open по focus/click/onChange + «Ничего не найдено» + мобильный dropdown right-0 + скроллбар чипов скрыт реальным классом `scrollbar-hide` (`scrollbar-none` в CSS не существовал); **Пульс освобождён от max_tokens** (директива владельца, guard от content=null остаётся); **OG-картинки хабов**: `/og/today.png` (сетка значений), `/og/region-rating/{code}.png` (барчарт топ-8), `/og/region-vs/{a}-vs-{b}.png` (таблица пары, компактные единицы) — `og_image.py::render_{rating,today_hub,region_vs}_og`, endpoints в `sitemap.py`, nginx-rewrites, og:image+ImageObject+видимый `<img>` в SSR-рендерах. Детали — `docs/backlog.md`. Ранее 2026-07-05 днём: правки по созвону «На правки 13» + замечания владельца: сравнение — drag-pan графика мышью + постоянный watermark; карта регионов — фикс поиска «Свой показатель» (dropdown обрезался `overflow-x-auto` контейнером чипов); crypto ETL — klines через зеркала Binance `data-api.binance.vision` (уход от 451); **Пульс: гипотезы не писались** — `max_tokens` 1400 мал, Bedrock при обрыве по длине отдаёт `content=null` → отчёт падал в fallback без блока `---HYPOTHESES---`; фикс: лимит→4000 + guard от null; всё на проде. Детали — `docs/backlog.md`. Ранее в тот же день — второй проход дня: **архив исходящих Telegram `telegram_outbox`** — каждая отправка (дайджест/алерт/кнопки/файлы) полностью сохраняется в БД с payload и байтами файлов, единая точка `services/telegram_outbox.py::archive()`, ADR-0009 п. 5 — это «глаза» агента следующей сессии; **UX-хром чистых SSR-страниц** (шапка-бренд + CTA + футер об источниках, одна точка `seo_renderer.py::_SSR_CHROME_*`) и **брендовая 404** (`render_not_found_html`); публичные тексты актуализированы: «100+ макроиндикаторов и 489 региональных показателей по 85 регионам» (2026-07-06: «96 территорий» — внутренняя размерность артефакта с РФ/ФО/агрегатами; наружу — 85 субъектов); `llms.txt` переписан под GEO/ИИ-поисковики; правила `working-agreement.mdc` пополнены: отчётность, репутация, «глаза и руки агента», цель 40k→100k→1M. Ранее в тот же день: спринт «10k визитов/день» — программа индексации 40k региональных URL + программатик-страницы под спрос, ADR-0003 «Subsequent additions»: единый реестр URL `app/services/site_urls.py` (одна точка истины для sitemap/IndexNow/переобхода, порядок секций = приоритет); sitemap-индекс `/sitemap.xml` → 12 секций (`/sitemap-{name}.xml`, регионы чанками 10k, nginx regex); IndexNow-батч всех ~43,3k URL выполнен 2026-07-04 (батчи ≤10k, скрипт `backend/scripts/indexnow-ping-all.py`); автоподача переобхода Вебмастера `webmaster_recrawl.py` (cron 09:10 МСК, ~150 URL/день, state Redis DB 1); новые SSR-семейства: `/today`+`/today/{code}` (10 ВЧ-кодов «X сегодня», `seo_today.py`), `/region-rating/{code}` (~489 рейтингов, `seo_regional.py`), `/region-vs/{a}-vs-{b}` (190 пар, `seo_region_compare.py`), `/calendar/{y}/{mm}` (`seo_calendar.py`); перелинковка главная→today/regions, регионы→рейтинги; фикс перевёрнутого BreadcrumbList регионального SSR. «Обход по счётчикам» — вручную в UI Вебмастера. Ранее 2026-07-03: DS-датасет расширен до трёх слоёв — ADR-0009 «Subsequent additions»: (1) retention поведенческого сырья ОТМЕНЁН (`behavior_raw_retention_days=0`, копим без удаления под Big Data/ML — директива владельца); (2) слой привлечения `metrika_acquisition.py` — ежедневный синк 08:20 МСК: повизитная выгрузка Logs API → `raw_metrika_visits` (фраза/источник/поисковик/реферер/UTM/гео/устройство/цели, 28 полей) + агрегаты Reporting API (traffic_sources/search_engines/referrers/ad_campaigns снапшоты, `metrika_search_phrases`, `metrika_daily_page_metrics`), Пульс-секция `acquisition` + `metrika_visits`/`metrika_ad_visits` в трендовой памяти; (3) булев слой знаний — таблица `hypotheses` (Alembic `20260703_hypotheses`): LLM-аналитик Пульса ведёт проверяемые гипотезы (вердикт true/false/открыта + confidence) через структурированный хвост ответа, кнопки бота «Датасет» (инвентаризация: строки/параметры/JSON-ключи по слоям, `dataset_inventory.py`) и «Гипотезы». Ранее 2026-07-03: ADR-0009 поведенческий поток first-party: `behavior.js` — автосбор без разметки (pageview на роут, каждый клик с иерархическим путём элемента + dead/rage-признаки, сэмплированная траектория мыши, dwell со скролл-глубиной, copy) → батчи `sendBeacon` на `/api/v1/analytics/behavior` → таблица `behavior_events` (Alembic `20260703_behavior`); дневные агрегаты в Пульс-снапшоте `snap["behavior"]` (топ страниц/кликов, dead/rage, dwell по страницам, копируемое) уходят LLM-ассистенту; прод переехал на 201.51.11.170 — DNS переключён, Caddy выпустил сертификат, Пульс включён с OpenRouter-ключом, телеграм-роутинг: дайджест владельцу+skrakan, realtime-алерты только владельцу, кнопки/CSV обоим (`interactive_authorized_ids`). Ранее 2026-07-02: региональный блок 2.0 + «Пульс» + auth: интерактивная SVG-карта регионов (choropleth, `RegionsMap.jsx` + `scripts/regional/build_map_paths.py`), сравнение регион-регион и dual-axis «— Россия» на карточке, экспорт CSV/Excel/PNG региональных рядов (auth-only) с watermark `forecasteconomy.com` везде, региональные ряды в /compare (`r:{slug}:{code}`), SEO-тексты+FAQ (JSON-LD) для ~38k региональных страниц, кросс-линки макро↔регионы (`RegionCrossLink`, `MACRO_BY_TABLE`), недельные цены на топливо с HTML-бюллетеня; **сессии изолированы от кэша** — state-Redis DB 1 (`get_state_redis`; sessions/lockout/quota/oauth-state), деплойный FLUSHDB больше не разлогинивает, sliding cookie через `/auth/me` (ADR-0007 Subsequent additions); **мониторинг «Пульс»** — дневные снапшоты всего (users/events/поиск/скачивания/ошибки/ETL) в Redis TTL 8 дней, LLM-отчёт через OpenRouter → Telegram владельцу, getUpdates-поллер с inline-кнопками (таблица/CSV пользователей, карточка-аналитика) — `services/{pulse,pulse_report,telegram_bot}.py`; почта `rebeka.ee@yandex.ru`. Плюс слитая с main работа 2026-07-01: ветка `feat/compare-view-modes`, локально: страница методологии прогнозирования `/methodology` — `frontend/src/pages/Methodology.jsx` + SSR `PAGE_META["methodology"]` + nginx-роут + help-подсказка у переключателя «Прогноз» на карточке (`IndicatorChartSection` → событие `methodology_click`); режим сравнения — второй проход по дефектам: безопасный ребейз «Общая база (=100)» (`isIndexableBase` — знакопеременные/%-ряды не индексируются, показываются нотой; снята коллизия «Индекс»-шкала vs «Индекс»-представление; подсказка при 3+ единицах); per-ряд переключатель представления Индекс·К прошлому периоду·К году через универсальный резолвер `frontend/src/lib/compareRepresentation.js` (generic sibling-коды из `viewModelFamilies.generated.json` + bespoke %-коды cpi/ppi/housing + client-transform'ы `sub100`/`mom`/`cpiCumulative`; накопленный индекс ИПЦ вынесен в общий `cpiCumulativeIndex.js`); прогнозы 5 квартальных баз (capital-investment/services-exports/services-imports/gdp-investment → generic_quarterly, fdi-net → signed_quarterly); annual-in-quarterly фикс `housing-qoq` через op `qoq_adjacent`; `wages-nominal-annual` автопродолжается движком (op `annual_mean_with_prefix`, ручной backfill-скрипт удалён) + gap-fill дыры 2022-12; +32 seo_keywords; About e-mail сотрудничества → rebeka.ee@yandex.ru. Детали — `docs/backlog.md`, ADR-0001 «Subsequent additions», `CONTEXT.md::Annual-in-quarterly trap`. Ранее 2026-06-30 (прогноз «К прошлому периоду» (М/м·Кв/Кв) ВОССТАНОВЛЕН для всех generic-семей с прогнозируемой месячной/квартальной базой (~140 кодов): derived от прогноза базы тем же generic-pipeline, что считает факт (mom поверх месячного прогноза; qoq — q/q на суммах/уровнях кварталов, неполный хвостовой квартал отбрасывает `_aggregate`); ранее был глобально отключён в `_mode_forecastable`/`_mode_forecast_meta` (созвон 2026-06-30: пустой тоггл «к прошлому периоду» во всех семьях — баг). weekly-базы (international-reserves, топливо) остаются без mom/qoq-прогноза (propagate-freq отсекает). 2026-06-25 (вечер: топливо переведено с недельного прогноза на месячный — `monthly_forecast` в `view_model_families`: avg-month → monthly_auto, квартал/год протягиваются из месячного; разделы ИПП (добыча/обработка/энергетика/водоснабжение) выведены отдельными карточками в «Бизнес и инвестиции» (variant-группа сохранена); «Индексы»→«Биржевые индексы», бензин→«Цены на бензин АИ-92/95», дизель→«Цена дизельного топлива», группа→«Цены на топливо»; OG-картинки переделаны в area-график (подписи осей в гаттерах, watermark); добавлен вечерний ETL-прогон 20:00 МСК + IndexNow; «О проекте» переписана (Росстат/ЦБ/Минфин, 100+ индикаторов); компаре-нудж «хотите больше двух — зарегистрируйтесь»; `scripts/verify-data-loaded.py`. Ранее: картинка-график индикатора теперь встроена ВИДИМЫМ `<img class="seo-chart">` в тело SSR + `ImageObject` + `og:image` — Алиса/Нейро берут график прямо из DOM страницы, не только из мета; картинка получила подписи осей min/max и крайние даты периода; **реальная зарплата `wages-real` выведена отдельной карточкой в категорию «Рынок труда»** — больше не скрыта, но остаётся в variant-группе «Заработная плата» переключателем с номинальной. Ранее: реализованы поголовые годовые OG-картинки `/og/{code}/{year}.png` + `ImageObject`-разметка годовых landing'ов для Алисы/Нейро; anti-orphan-инвариант для `is_listed=false`; реальная зарплата объединена со средней в variant-группу «Заработная плата» + собственная T8-семья + «О показателе» + прогноз + годовая частота 1991+; компаре-поиск трекает спрос с числом результатов; новые цели Метрики задокументированы в `analytics_api_inventory/frontend_instrumentation.md`. Ранее: добавлен FAST-PATH «новый ИНДИКАТОР» — канонический 9-пунктный рецепт владельца + автозанос в поиск + спецификация материалов для Яндекс.Алисы; ИПП объединён в variant-группу «Состав промышленного производства» (общий + 4 раздела), топливо — в «Автомобильное топливо» (АИ-92/95/дизель); «О показателе» заведены для ИПП-разделов и топлива (DRY-билдеры); глобальный поиск и поиск сравнения листают весь каталог без «топ-N»; экспорт картинки графика — фон из реальной светлой темы (фикс тёмного PNG), правило watermark унифицировано «гость ⇒ знак, зарегистрирован ⇒ чисто». Ранее: FAST-PATH «новая КАТЕГОРИЯ» — 6 точек касания; категории «Индексы»/«Товарные рынки»; крипта ETH/SOL; гостевой лимит выгрузок = 0).
...
Задача проекта в одну строку: публичная аналитическая платформа `forecasteconomy.com` — официальная макроэкономическая статистика по странам (национальные ведомства, Евростат, МВФ; по России — глубокое покрытие Росстата, ЦБ и Минфина), derived-ряды и forecast'ы, фронтенд + боты + embed. Английский канон — apex, русский — `ru.forecasteconomy.com`.
...
5. Закоммитить сразу после завершённой правки (диктовка 2026-09-04). Push и деплой — только по явной команде.
