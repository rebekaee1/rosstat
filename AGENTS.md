...
- **Что показывает ассистент:** текст-сниппет (meta description / структурный
  контент) + одну картинку. Картинка берётся из `og:image` страницы или из
  размеченного `schema.org/ImageObject`, поэтому картинка должна быть
  **самодостаточной** (заголовок, ось, последнее значение, источник, бренд
  `forecasteconomy.com`) — тогда, показанная отдельно, она объясняет себя и
  разносит бренд.
