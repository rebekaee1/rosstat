...
| 2b | **Initial ETL trigger** | После добавления нового source-индикатора в `seed_data.py` — обязательно ручной `await run_etl_for_indicator(<code>)` через `docker compose exec backend python3 -c "..."`. Daily-job ходит раз в сутки в 06:00 МСК — между deploy и первым ETL индикатор стоит пустой. См. trap `New indicator initial ETL trap`. |
