...
- **2026-05-07 — FAQ «ставка по вкладам»:** пользователь спросил источник данных для категории «Процентные ставки». Ответ: индикатор `deposit-rate` («Ставка по вкладам»), категория БД `Ставки` — **Банк России**, ETL через `cbr_dataservice_json` (DataService `publicationId=18`, `datasetId=37`, `measureId=2`, `element_id=7`). **Уточнение:** старый human-facing URL `cbr.ru/statistics/pdko/int_rat/` отдаёт **404**; актуальный хаб — `cbr.ru/statistics/bank_sector/int_rat/` (XLSX остаются под `/vfs/statistics/pdko/int_rat/`). В `seed_data.py` все `source_url` на этот блок переведены на новый хаб.
- **2026-05-07 — Валидация 2022 по вкладам + объяснение низких средних:** пользователь: 404 на ссылку источника и сомнение в уровнях 3–5% с 2022 г. Проверено: ряд на проде **побитово совпадает** с ответом `GET https://www.cbr.ru/dataservice/data?publicationId=18&datasetId=37&measureId=2&element_id=7` (пример: янв. 2022 = 5.11%). За 2022 год ряд **не плоский**: март **18.67%**, апрель **13.87%**, затем снижение к ~5–8% — это официальная средневзвешенная по объёму привлечённых вкладов ФЛ в рублях до года включая «до востребования», не ключевая ставка и не «максимальная ставка по вкладу» из маркетинга банков.
...
- **2026-05-07 — Прогноз ВВП во всех 8 индикаторах + Footer «Инструменты» + SEO для /calendar:** на проде после деплоя `1873e13` (split nominal/real GDP) три из восьми индикаторов отдавали `forecast: null`: `gdp-nominal-annual`, `gdp-real-yoy`, `gdp-real-qoq`; четвёртый (`gdp-real`) держал старую derived-модель `GDP-Real-From-YoY-Derived`, потому что `--forecast-only` retrain после деплоя не запускался. Корень: автоматическая генерация прогнозов после `git pull` срабатывает только при `seed()`, а entrypoint обычно его не вызывает; новые derived seed'ются, но retrain не пробрасывается. **Действия:** через SSH/Docker exec запущен прицельный retrain в правильном порядке (источники → derived) — все 8 кодов получили валидные модели за один проход (gdp-real теперь `GDP-Real-Quarterly-MW`, остальные derived — `GDP-Nominal-YoY/QoQ-Derived`, `GDP-Real-YoY/QoQ-Derived`, `GDP-Nominal-Annual-Sum`, `GDP-Real-Annual-Sum`); Redis-кэш `fe:*:forecast` сброшен. **Тесты-контракт:** `backend/tests/test_forecast_policy.py` — добавлен `test_all_gdp_family_indicators_have_active_forecast_config` (8 индикаторов обязаны попасть в любой forecast-track + `forecast_steps>0`); `backend/tests/test_gdp_forecast_api.py` — новый интеграционный API-тест с TestClient (skip'ается, если БД недоступна — 8 параметризованных кейсов). Зелёные.
  - **Footer:** добавлена 5-я колонка «Инструменты» с тремя ссылками: Календарь публикаций (`/calendar`), Сравнение индикаторов (`/compare`), Калькулятор инфляции (`/calculator`). Сетка `lg:grid-cols-5` ровно заполняется (бренд + 4 раздела). На мобильном (`sm:grid-cols-2`) ложится в две строки 2×2 после баннера бренда. Все ссылки используют общий `footLink` (focus-ring, lift-hover) — без иконок, минимализм.
  - **SEO `/calendar`:** на проде title/description/canonical/og:* уже отдавались уникально (через backend `seo_renderer` + `PAGE_META["calendar"]`), но в sitemap.xml (как в backend `STATIC_PAGES`, так и в статическом fallback `frontend/public/sitemap.xml`) `/calendar` отсутствовал. Добавил с `changefreq=weekly`, `priority=0.7`. Robots.txt — не блокирует (Allow: /). Двойная видимость (Header + Footer) для всех важных страниц подтверждена.
  - **Качество:** `pytest tests/test_seo_og.py tests/test_forecast_policy.py tests/test_calculation_engine.py tests/test_derived_ops.py tests/forecast_strategies/` — 74 passed; `npm run lint` clean; `vitest` 40/40; `vite build` 3.41s.
  - **Не трогал** `backend/app/services/calendar_seed.py`, `backend/app/api/calendar.py`, `backend/app/main.py`, `backend/entrypoint.sh`, `backend/seed_data.py` — всё это в работе у параллельного воркера на ветке `Calendar audit + completeness`.
  - **User intention:** «у кого-то есть прогноз, у кого-то нет — нужно, чтобы везде были» + футер не должен скрывать инструменты + SEO перепроверить. **Reaction (ожидается):** одобрение и подтверждение что прогнозы видны во всех 8 вкладках при переключении.

- **2026-05-07 — Календарь: rolling 12-мес, еженедельные события, авто-обновление (commit `d46b9c9`).** Никита Александрович: «час назад на 7 мая видел Международные резервы, сейчас ничего нет. Проверь источники и расставь правильно». Аудит выявил три корневые причины: (1) `calendar_seed.py` всегда генерировал жёсткий 2026-год с `range(4,13)`, без rolling-окна — на 2027 пусто; (2) ни в `entrypoint.sh`, ни в scheduler не было запуска seed — на проде он отрабатывал ровно один раз (когда фича выкатывалась), и более поздние правки `CBR_STAT_MONTHLY` (M2/резервы) не дошли в БД (18 событий не вставились); (3) еженедельные события полностью отсутствовали — Международные резервы (четверг, СCРД МВФ), Денежная база узкая (пятница), недельный ИПЦ (среда). На проде на 60 дней было 73 события, на 14 дней рядом — всего 19, на 7 мая 0.
  - **Расширение `backend/app/services/calendar_seed.py`** (коммит +570 строк, −127): `seed_calendar(months_ahead=12, today=None)` теперь rolling-окно `[today−14d, today+12mo]`. `_build_monthly_release_events` итерирует по факт. месяцам внутри окна (а не `range(4,13)`); `_build_quarterly_release_events` строится от quarter_end + lag_days, а не вручную; новый `_build_weekly_events` создаёт по одному событию в неделю на каждый `WeeklySpec` (3 spec'а: Thursday Reserves CBR imp=2, Friday Monetary Base CBR imp=1, Wednesday Weekly CPI Rosstat imp=2). Расширены месячные публикации: `ppi`, `construction-work`, `external-debt`, `current-account`. Квартальные: `gdp-nominal` (Q+60d) + `gdp-real` (Q+90d). Добавлены 2027 CBR-meetings (3 первых, `is_estimated=True` — официальный календарь публикуется в декабре 2026). Идемпотентность: `on_conflict_do_nothing` по `uq_event_natural_key`; для weekly-без-`indicator_id` (где NULL=NULL не считается равным) — pre-check по `(source, event_type, scheduled_date, title)` перед insert; finalizer `_cleanup_legacy_orphans` сносит дубли «orphan c indicator_id IS NULL» когда есть линкованный аналог (закрывает наследие старых seed'ов с устаревшими кодами `money-m2`/`reserves-gold-fx`); `_cleanup_orphan_duplicates` — keep-min-id для NULL-строк.
  - **Исправление кодов:** `money-m2 → m2`, `reserves-gold-fx → international-reserves` (старые имена в БД отсутствовали, поэтому события вставлялись с `indicator_id=NULL` и не показывались с привязкой к индикатору).
  - **Авто-обновление:** `backend/entrypoint.sh` теперь между `seed_data.py` и `uvicorn` гонит `python -m app.services.calendar_seed` (3-й шаг из 4-х). В `backend/app/main.py` зарегистрирован APScheduler-job `calendar_refresh` — `CronTrigger(day=1, hour=3, minute=0, timezone='Europe/Moscow')` — в начале каждого месяца окно подкатывается на ~1 месяц вперёд.
  - **Тесты:** новый `backend/tests/test_calendar_seed.py` (10 кейсов): минимум 200 событий в 12-мес окне; importance ∈ {1,2,3}; в next 30 days ≥ 25 событий и сразу 3 источника `{cbr, rosstat, minfin}`; на каждый четверг next 30 days существует «Международные резервы РФ» (cbr, imp=2, time=16:00); каждый WeeklySpec.weekday совпадает с фактическим weekday вставленных событий; past события status=released, future — scheduled; 2027 meetings — `is_estimated=True`, 2026 — False; user-visible поля (title/title_en/description/reference_period) НЕ содержат внутренний жаргон (SARIMA / Никита / ноутбук / notebook / Holt-Winters / OLS / multi-window). Полный backend `pytest -q --ignore=tests/test_rosstat_labor`: **208 passed**.
  - **Деплой `b56d3ed → d46b9c9`:** `git pull --ff-only && git push origin main`, затем `bash /opt/rosstat/scripts/deploy.sh` на 5.129.204.194: образы пересобраны, контейнеры здоровы, alembic миграций нет (схема не двигалась). Entrypoint автоматически отработал `seed_calendar`: **287 events inserted** на проде. APScheduler принял `calendar_refresh` (1-го числа 03:00 МСК). Redis-кэш `fe:calendar:*` сброшен.
  - **Прод-проверка:**
    - `https://forecasteconomy.com/api/v1/calendar?from=2026-05-07&to=2027-05-07` → **349 событий**, источники: ЦБ 97 / Росстат 89 / Минфин 14, importance: 1=51, 2=134, 3=15.
    - `/api/v1/calendar/upcoming?limit=10&importance_min=1` (на 7 мая) → первое событие `2026-05-07 cbr imp=2 Международные резервы РФ`, далее 8 мая (рез. ежемес.), 12 мая (резюме ставки), 13 мая (нед. ИПЦ Росстат), 14 мая (рез. еженед.), 15 мая (4 события: рознич. торговля, ввод жилья, ден. база), 17/18/20/21 мая.
    - Браузер `cursor-ide-browser` на `forecasteconomy.com/calendar`: Hero — **«Ближайшее событие — Международные резервы РФ — четверг, 7 мая 2026 г. 16:00 МСК»** + ссылка на индикатор Международные резервы; сетка май 2026 показывает плотные точки на каждый четверг (CBR резервы), пятницу (CBR денежная база), среду (Росстат недельный CPI); 7 мая выделен как сегодня (золотой кружок). Console — без ошибок приложения.
  - **User intention:** «всё актуальное и правильное, сверяться с официальными источниками — это очень важно». Ответ: расписание Международных резервов РФ ЦБ — еженедельно по четвергам 16:00 МСК (СCРД МВФ); 7 мая 2026 четверг — событие появилось. Денежная база (узкая) — пятница 11:00. Недельный ИПЦ Росстата — каждую среду. На 12 мес вперёд календарь теперь живой и автообновляется без ручного вмешательства.
  - **Future work:** парсер официального ЦБ vCalendar `cbr.ru/Queries/FileSource/96347/vCalendar.ics` — точные даты (с учётом смещений из-за праздников: например, в мае-2026 reserves weekly смещены с четверга 7 мая на пятницу 8 мая из-за длинных майских; наш генератор этого не знает и ставит на четверг 7 мая, что тоже валидно — публикация всё равно происходит на этой неделе). Роли отдельной задачей.
