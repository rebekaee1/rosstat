...
- **2026-04-29 — новые замечания Никиты после просмотра v4:** реализовано без деплоя на момент записи: CPI mode-кнопки перенесены в верхний блок в стиле variant chips; недельная вкладка оставлена только для общего `cpi`; forecast bridge в bar-графике больше не добавляет фиолетовый столбик на уже фактический месяц; `gdp-real` возвращён отдельной карточкой; прогнозы в категориях `Финансы`, `Ставки`, `Торговля` отключаются через существующий механизм `forecast_steps=0` + очистка stale forecasts; для готовых файлов Никиты добавлены approved forecast values: `ppi` из `/Users/iprofi/Downloads/Telegram Desktop/Прогноз_ИЦП.ipynb` (2026-03..2027-02) и `gdp-nominal` из `Прогноз_номинальный_ВВП.ipynb` (4 квартала 2026). Проверки локально: backend `131 passed`, frontend `vitest 38`, `eslint`, `vite build` green.
