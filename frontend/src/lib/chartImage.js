...
function drawWatermark(ctx, w, h) {
  ctx.save();
  // Диагональная плитка — мягкая, не мешает читать график.
  ...
  // Чёткая подпись-«копирайт» в правом нижнем углу — всегда читаемая.
  ctx.save();
  ctx.globalAlpha = 0.9;
  ...
  ctx.fillStyle = 'rgba(214, 188, 138, 0.85)';
 ...
 */
export async function exportNodeToPng(node, { filename = 'chart.png', watermark = true } = {}) {
