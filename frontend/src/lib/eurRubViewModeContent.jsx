/**
 * Описание и методология: курс евро EUR/RUB (режим × частота ряда).
 * Правила: .cursor/rules/methodology-language.mdc
 */

import { isEurRubFamily } from './eurRubViewModeResolve';

function contentLevel() {
  return {
    description:
      'На графике — официальный курс евро к рублю в рублях за один евро: '
      + 'значение, которое Банк России устанавливает ежедневно на основе '
      + 'итогов валютного рынка. Линия меняется изо дня в день и отражает, '
      + 'сколько рублей стоит евро в расчётах и отчётности, а не биржевую '
      + 'котировку в реальном времени.',
    methodology:
...
function contentAgg(mode) {
  ...
  return {
    description:
      `На графике — средний официальный курс евро внутри календарных `
...
export function getEurRubChartTitle(chartMode) {
  switch (chartMode) {
    case 'weekly':
      return 'Курс евро — среднее по неделям (руб.)';
    case 'monthly':
      return 'Курс евро — среднее по месяцам (руб.)';
    case 'quarterly':
      return 'Курс евро — среднее по кварталам (руб.)';
    case 'annual':
      return 'Курс евро — среднее по годам (руб.)';
    default:
      return 'Курс евро (EUR/RUB)';
  }
}

export function getEurRubTableTitle(chartMode) {
  switch (chartMode) {
    case 'weekly':
    ...
    default:
      return 'Исторические данные — EUR/RUB';
  }
}

export function getEurRubViewModeContent({ chartMode = 'level' }) {
