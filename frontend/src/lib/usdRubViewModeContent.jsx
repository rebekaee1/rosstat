/**
 * Описание и методология: курс доллара USD/RUB (режим × частота ряда).
 * Правила: .cursor/rules/methodology-language.mdc
 */

import { isUsdRubFamily } from './usdRubViewModeResolve';

function contentLevel() {
  return {
    description:
      'На графике — официальный курс доллара США к рублю в рублях за один '
      + 'доллар: значение, которое Банк России устанавливает ежедневно на '
      + 'основе итогов валютного рынка. Линия меняется изо дня в день и '
      + 'отражает, сколько рублей стоит доллар в расчётах и отчётности, '
      + 'а не биржевую котировку в реальном времени.',
    methodology:
...
function contentAgg(mode) {
  ...
  return {
    description:
      `На графике — средний официальный курс доллара внутри календарных `
...
export function getUsdRubChartTitle(chartMode) {
  switch (chartMode) {
    case 'weekly':
      return 'Курс доллара — среднее по неделям (руб.)';
    case 'monthly':
      return 'Курс доллара — среднее по месяцам (руб.)';
    case 'quarterly':
      return 'Курс доллара — среднее по кварталам (руб.)';
    case 'annual':
      return 'Курс доллара — среднее по годам (руб.)';
    default:
      return 'Курс доллара США (USD/RUB)';
  }
}

export function getUsdRubTableTitle(chartMode) {
  switch (chartMode) {
    case 'weekly':
    ...
    default:
      return 'Исторические данные — USD/RUB';
  }
}

export function getUsdRubViewModeContent({ chartMode = 'level' }) {
