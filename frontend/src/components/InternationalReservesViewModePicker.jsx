...
import {
  ...
  getTopGroup,
  highlightedTopGroup,
} from '../lib/internationalReservesViewModeGroups';
...
export default function InternationalReservesViewModePicker({
  currentMode,
  onChange,
  trackContext,
  compact = false,
  ...
  const trackMode = useCallback((mode, groupId) => {
    track(events.CHART_MODE_CHANGE, {
      mode,
      internationalReservesViewGroup: groupId,
  ...
  const body = (
    <>
      ...
      <div className="flex flex-wrap gap-2">
        {INTERNATIONAL_RESERVES_TOP_GROUPS.map((group) => {
