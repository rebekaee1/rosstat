...
import {
  ...
  getTopGroup,
  highlightedTopGroup,
} from '../lib/householdFinanceViewModeGroups';
...
export default function HouseholdFinanceViewModePicker({
  currentMode,
  onChange,
  trackContext,
  compact = false,
  ...
  const trackMode = useCallback((mode, groupId) => {
    track(events.CHART_MODE_CHANGE, {
      mode,
      householdFinanceViewGroup: groupId,
  ...
  const body = (
    <>
      ...
      <div className="flex flex-wrap gap-2">
        {HOUSEHOLD_FINANCE_TOP_GROUPS.map((group) => {
