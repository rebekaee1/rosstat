...
try {
  for (const [name, path, width] of [['home-mobile', '/', 390], ['fast-year-mobile', '/russia/indicator/cpi/2025', 390], ['indicator-mobile', '/russia/indicator/cpi', 390], ['world-desktop', '/germany', 1440]]) {
    ...
    await page.addInitScript(() => {
      ...
      new PerformanceObserver(list => { for (const e of list.getEntries()) if (!e.hadRecentInput) window.__localVitals.shifts.push({ time: e.startTime, value: e.value }); }).observe({ type: 'layout-shift', buffered: true });
    ...
    const measurement = await page.evaluate(() => {
      ...
      let max = 0, current = 0, start = 0, prev = 0;
      for (const shift of v.shifts) {
        if (shift.time - prev > 1000 || shift.time - start > 5000) { start = shift.time; current = 0; }
    ...
    report.cases.push({ name, path, width, ...measurement, errors, badResponses });
...
fs.writeFileSync('docs/design/local-acceptance/performance-lab.json', JSON.stringify(report, null, 2)+'\n');
