...
Источник: https://www.cbr.ru/hd_base/metall/metall_base_new/
HTML-таблица UniDbQuery: дата | Золото | Серебро | Платина | Палладий (руб/грамм).
"""
...
METAL_COL = {"gold": 0, "silver": 1, "platinum": 2, "palladium": 3}
...
def fetch_gold_html(date_from: date, date_to: date) -> tuple[str, str]:
    url = f"{settings.cbr_base_url.rstrip('/')}/hd_base/metall/metall_base_new/"
    params = {
...
def parse_gold_html(html: str, metal_col: int = 0) -> list[tuple[date, float]]:
    """Parse CBR metals HTML table.

    metal_col: 0=gold, 1=silver, 2=platinum, 3=palladium
    """
    ...
    for row_html in rows:
        ...
        try:
            ...
            val = _parse_ru_float(cells[1 + metal_col])
...
class CbrGoldParser(BaseParser):
    ...
    async def run(self, db: AsyncSession, indicator: Indicator, fetch_log: FetchLog) -> None:
        ...
        try:
            ...
            metal_col = METAL_COL.get(metal, 0)
            date_to = date.today()
            ...
            while chunk_start < date_to:
                ...
                try:
                    html, final_url = await asyncio.to_thread(fetch_gold_html, chunk_start, chunk_end)
                    chunk_points = await asyncio.to_thread(parse_gold_html, html, metal_col)
