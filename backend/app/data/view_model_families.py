...
_FAMILY_DEFS: list[FamilyDef] = [
    ...
    # T10a — знакопеременное сальдо / доля к ВВП: уровень + Г/г в п.п.
    FamilyDef(
    ...
    FamilyDef(
        ...
        "Государственный долг сектора государственного управления",
        "T10a", "% ВВП", "Государственные финансы", "annual",
        yoy_unit="п.п.", forecastable=False,
    ),
]
...
def _mode_forecastable(
    ...
    if m.is_native:
        return True
    # «К прошлому периоду» (М/м, Кв/Кв), «Г/г» и агрегаты прогнозируемы, если
    # частота базы протягивает прогноз в агрегаты (`_FORECAST_PROPAGATE_FREQ`).
    # mom/qoq-прогноз — детерминированная производная прогноза базы (та же op,
    # что считает факт): на сезонном ряде он «пилит» ровно как исторический
    # mom/qoq — это корректное продолжение сезонности, а не шум. Созвон
    # 2026-06-30: «К прошлому периоду» обязан показывать прогноз (раньше он был
    # глобально отключён — пользователь увидел пустой тоггл во всех семьях).
...
def _mode_forecast_meta(fam: "Family", m: "Mode", base_freq: str) -> dict | None:
    ...
    if base_freq not in _FORECAST_PROPAGATE_FREQ:
        return None
    # mom/qoq тоже получают derived-прогноз через тот же generic-pipeline, что и
    # факт: mom — поверх месячного прогноза базы (первая точка считается от
    # последнего факта); qoq — q/q на суммах/уровнях кварталов, неполный
    # хвостовой квартал отбрасывает `_aggregate`, текущий достраивается
    # фактом+прогнозом. См. `_mode_forecastable` (созвон 2026-06-30).
