...
def test_credit_indicator_dataservice_config(code: str) -> None:
    ...
    # Месячный прогноз руководителя (monthly_auto) включён для всех месячных
    # ставок, включая кредитные (Прогноз_месячных_данных.ipynb, 2026-06).
    assert cfg["forecast_steps"] == 12
    assert cfg["forecast_strategy"] == "monthly_auto"
