...
# Аргументы (`docker compose run … backend true`) — одноразовый прогон
# миграций и seed без второго uvicorn. Иначе preboot деплоя поднял бы
# второй планировщик рядом с живым сервером. Обычный `up` аргументов не
# передаёт и стартует uvicorn.
if [ "$#" -gt 0 ]; then
  exec "$@"
fi
