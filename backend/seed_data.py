...
INDICATORS = [
    ...
    },
    {
        ...
        "methodology": (
            "Индекс цен на продовольственные товары относительно предыдущего месяца: "
            "100 означает, что цены на продукты питания не изменились. Считает Росстат "
            "по той же сети наблюдения и весам корзины, что и общий индекс потребительских "
            "цен, но только по продовольственной группе. Режимы на карточке (инфляция "
            "за 12 месяцев, к предыдущему периоду, индекс) строятся из этого официального ряда."
        ),
        "parser_type": "rosstat_cpi_xlsx",
        "model_config_json": {"forecast_steps": 12},
        "is_active": True,
    ...
    },
    {
        ...
        "methodology": (
            "Индекс цен на непродовольственные товары относительно предыдущего месяца. "
            "Охват совпадает с непродовольственной частью потребительской корзины Росстата: "
            "одежда, товары длительного пользования, топливо на АЗС и прочие непродовольственные "
            "товары без услуг. Режимы на карточке преобразуют тот же официальный месячный ряд."
        ),
        "parser_type": "rosstat_cpi_xlsx",
        "model_config_json": {"forecast_steps": 12},
        "is_active": True,
    ...
    },
    {
        ...
        "methodology": (
            "Индекс цен на потребительские услуги относительно предыдущего месяца. "
            "В корзину входят жилищно-коммунальные услуги, транспорт, связь, образование, "
            "здравоохранение и другие платные услуги населению. Считает Росстат; режимы "
            "на карточке строятся из этого официального ряда."
        ),
        "parser_type": "rosstat_cpi_xlsx",
        "model_config_json": {"forecast_steps": 12},
        "is_active": True,
    ...
    },
    {
        ...
        "methodology": (
            "Средневзвешенная ставка по новым рублёвым вкладам физических лиц сроком "
            "до одного года, включая вклады до востребования, в процентах годовых. "
            "Веса задаются объёмами привлечения за отчётный месяц. Публикует Банк России. "
            "На карточке — уровень ставки; соседние сроки доступны переключателем."
        ),
        "parser_type": "cbr_dataservice_json",
        "model_config_json": {
            "dataservice": {
    ...
    # ─── Производные (CalculationEngine) ───
    {
        ...
        "methodology": (
            "Ряд расчётный: номинальная начисленная заработная плата делится на "
            "индекс потребительских цен и приводится к базе 100 в январе 2015 года. "
            "Значение показывает уровень покупательной способности средней зарплаты "
            "относительно января 2015 года, а его изменение к тому же месяцу "
            "прошлого года — на сколько процентов выросла или снизилась реальная "
            "зарплата за год. Росстат публикует официальный индекс реальной "
            "заработной платы накопленным итогом с начала года, поэтому значения "
            "здесь могут отличаться от официальных публикаций за те же месяцы. "
            "Источник исходных данных — Росстат."
        ),
        "parser_type": "derived",
        "model_config_json": {
    ...
    },
    {
        ...
        "methodology": (
            "Остаток депозитов нефинансовых организаций в банках на конец месяца, "
            "миллиарды рублей: переводные, срочные и валютные вклады в одном агрегате. "
            "Публикует Банк России. На карточке — помесячный остаток и средние за квартал "
            "и год из того же ряда."
        ),
        "parser_type": "cbr_monetary_agg_xlsx",
        "model_config_json": {
            "monetary_agg": {
    ...
    # ─── Внешняя торговля (ЦБ DataService — BOP) ───
    {
        ...
        "methodology": (
            "Сальдо счёта текущих операций за квартал в миллионах долларов США: "
            "товары, услуги, первичная и вторичная доходность. Положительное значение — "
            "профицит, отрицательное — дефицит. Считает Банк России по методологии "
            "платёжного баланса. На карточке — квартальный уровень и сумма за год."
        ),
        "parser_type": "cbr_dataservice_json",
        "model_config_json": {
            "dataservice": {
    ...
    # alternate_frequencies → linkage на monthly counterpart (T3 plan, frequency switcher).
    {
        ...
        "methodology": (
            "Стоимость товарного экспорта за квартал в миллионах долларов США "
            "по методологии платёжного баланса, а не только таможенных деклараций. "
            "Публикует Банк России. На карточке — квартальный объём, сумма за год "
            "и темпы к предыдущему периоду из того же ряда."
        ),
    ...
    },
    {
        ...
        "methodology": (
            "Стоимость товарного импорта за квартал в миллионах долларов США "
            "по методологии платёжного баланса. Публикует Банк России. На карточке — "
            "квартальный объём, сумма за год и темпы к предыдущему периоду."
        ),
        "parser_type": "cbr_bop_xlsx",
        "model_config_json": {
            "bop_target": "imports",
    ...
    },
    {
        ...
        "methodology": (
            "Разность товарного экспорта и импорта за квартал в миллионах долларов США. "
            "Положительное значение — профицит внешней торговли товарами. Считает "
            "Банк России по методологии платёжного баланса. На карточке — квартальное "
            "сальдо и сумма за год."
        ),
        "parser_type": "cbr_bop_xlsx",
        "model_config_json": {
    ...
    # ─── Услуги BOP (ЦБ BOP XLSX) ───
    {
        ...
        "methodology": (
            "Стоимость экспорта услуг за квартал в миллионах долларов США "
            "по методологии платёжного баланса: транспорт, поездки, телекоммуникации, "
            "финансовые и прочие услуги. Публикует Банк России. На карточке — "
            "квартальный объём и сумма за год."
        ),
        "parser_type": "cbr_bop_xlsx",
        "model_config_json": {
    ...
    },
    {
        ...
        "methodology": (
            "Стоимость импорта услуг за квартал в миллионах долларов США "
            "по методологии платёжного баланса. Публикует Банк России. На карточке — "
            "квартальный объём и сумма за год из того же ряда."
        ),
        "parser_type": "cbr_bop_xlsx",
        "model_config_json": {
    ...
    },
    {
        ...
        "methodology": (
            "Чистый приток прямых иностранных инвестиций за квартал в миллионах "
            "долларов США по финансовому счёту платёжного баланса: приобретение "
            "нерезидентами долей и долговых инструментов российских компаний за "
            "вычетом изъятий. Это поток за квартал, а не накопленный запас. "
            "Публикует Банк России. Знак может меняться."
        ),
        "parser_type": "cbr_bop_xlsx",
        "model_config_json": {
            "bop_target": "fdi-net",
    ...
    # ─── Демография (Росстат) ───
    {
        ...
        "methodology": (
            "Учитываются все живорождения, зарегистрированные органами записи актов "
            "гражданского состояния на территории России за календарный год. "
            "Значение за последний год приводится по оперативным данным помесячной "
            "регистрации и уточняется после публикации годовых итогов — расхождение "
    ...
    },
    {
        ...
        "methodology": (
            "Учитываются все случаи смерти, зарегистрированные органами записи актов "
            "гражданского состояния на территории России за календарный год. "
            "Значение за последний год приводится по оперативным данным помесячной "
            "регистрации и уточняется после публикации годовых итогов — расхождение "
            "обычно составляет доли процента."
    ...
    # ─── Розничная торговля (Росстат ежемесячный сборник) ───
    {
        ...
        "methodology": (
            "Оборот розничной торговли в текущих ценах за календарный месяц, "
            "миллиарды рублей: продажа товаров населению через торговую сеть "
            "и вне магазинов. Публикует Росстат. На карточке — помесячный объём "
            "и суммы за квартал и год из того же ряда."
        ),
    ...
    # ─── Ввод жилья (Росстат ежемесячный сборник) ───
    {
        ...
        "methodology": (
            "Общая площадь жилых домов, введённых в действие за календарный месяц, "
            "миллионы квадратных метров. Учитываются многоквартирные и индивидуальные "
            "дома всех форм собственности. Публикует Росстат. На карточке — помесячный "
            "ввод и суммы за квартал и год."
        ),
        "parser_type": "rosstat_ind_monthly",
        "model_config_json": {
            "ind_sheet": "1.8 ",
    ...
    # ─── Степень износа основных фондов ───
    {
        ...
        "methodology": (
            "Отношение накопленного износа основных фондов к их полной учётной "
            "стоимости на конец года, в процентах. Чем выше значение, тем больше "
            "доля уже списанной стоимости. Публикует Росстат ежегодно."
        ),
    ...
    # ─── Наука и образование ───
    {
        ...
        "methodology": (
            "Число аспирантов на начало учебного года, человек. Учитываются "
            "организации, ведущие подготовку научно-педагогических кадров. "
            "Публикует Росстат ежегодно."
        ),
    ...
    },
    {
        ...
        "methodology": (
            "Число докторантов на начало учебного года, человек. Публикует Росстат "
            "ежегодно по организациям, ведущим подготовку кадров высшей квалификации."
        ),
        "parser_type": "rosstat_science",
        "model_config_json": {"forecast_steps": 0},
        "is_active": True,
    ...
    },
    {
        ...
        "methodology": (
            ...
            "в отчётном году, единиц. Публикует Росстат ежегодно."
        ),
        "parser_type": "rosstat_science",
        "model_config_json": {"forecast_steps": 0},
        "is_active": True,
    ...
    },
    {
        ...
        "methodology": (
            ...
            "человек, на отчётную дату года. Публикует Росстат ежегодно."
        ),
        "parser_type": "rosstat_science",
        "model_config_json": {"forecast_steps": 0},
        "is_active": True,
    ...
    },
    {
        ...
        "methodology": (
            "Доля организаций, осуществлявших инновационную деятельность, "
            "в общем числе обследованных организаций, в процентах. С 2018 года "
            "считается по обновлённой методике Росстата и не сопоставляется "
            "с более ранним рядом. Публикация ежегодная."
        ),
        "parser_type": "rosstat_science",
        "model_config_json": {"forecast_steps": 0},
        "is_active": True,
    ...
    },
    {
        ...
        "methodology": (
            "Доля организаций, осуществлявших технологические инновации в отчётном году, "
            "в процентах. С 2018 года методика Росстата обновлена и не сопоставляется "
            "с более ранней историей. Публикация ежегодная."
        ),
        "parser_type": "rosstat_science",
        "model_config_json": {"forecast_steps": 0},
        "is_active": True,
    ...
    },
    {
        ...
        "methodology": (
            "Доля малых предприятий, осуществлявших инновационную деятельность "
            "в отчётном году, в процентах от числа обследованных малых предприятий. "
            "Публикует Росстат ежегодно."
        ),
        "parser_type": "rosstat_science",
        "model_config_json": {"forecast_steps": 0},
    ...
    },
    {
        ...
        "description": (
            ...
            "криптовалюта; ряд используют как ориентир по аппетиту к риску "
            "наряду с золотом, валютой и сырьевыми ценами."
        ),
        "methodology": (
    ...
    },
    {
        ...
        "description": (
            ...
            "основная платформа для смарт-контрактов; ряд сопоставляют с "
            "биткоином и другими долларовыми активами."
        ),
        "methodology": (
        ...
        "model_config_json": {
            ...
            "backfill_from": "2016-05-18",
            "pre_binance": {"provider": "coinbase", "symbol": "ETH-USD"},
    ...
    },
    {
        ...
        "unit": "пунктов",
        "frequency": "daily",
        "source": "Совет управляющих Федеральной резервной системы США",
        "source_url": "https://www.federalreserve.gov/releases/h10/current/",
        "description": (
            "Широкий номинальный индекс доллара США к корзине валют "
            "основных торговых партнёров. База расчёта — январь 2006 года "
            "равно 100. Показатель отражает внешнюю стоимость доллара и "
            "используется вместе с сырьевыми ценами и доходностью "
            "гособлигаций США."
        ),
        "methodology": (
            "Ежедневный номинальный широкий индекс доллара США из релиза "
            "H.10 Совета управляющих Федеральной резервной системы. Каждая "
            "точка — значение за рабочий день; выходные и праздники в ряд "
            "не входят. На карточке доступны дневное значение и средние по "
            "неделе, месяцу, кварталу и году из того же ряда; прогноз не "
            "строится."
        ),
    ...
    },
    {
        ...
        "description": (
            "Доходность 10-летних казначейских облигаций США — базовый "
            "ориентир долгосрочных ставок на мировом рынке капитала. Рост "
            "доходности обычно означает ужесточение финансовых условий; "
            "снижение — смягчение. Ряд сопоставляют с индексом доллара и "
            "сырьевыми ценами."
        ),
        "methodology": (
            "Ежедневная доходность постоянно действующих 10-летних "
            "казначейских облигаций США по данным Министерства финансов "
            "США. Каждая точка — значение за торговый день в процентах "
            "годовых; в выходные и праздники точек нет. На карточке доступны "
            "дневное значение и средние по периодам из того же ряда; прогноз "
            "не строится."
        ),
        "parser_type": "fred_csv",
        "model_config_json": {
    ...
    {
        "code": "eur-usd",
        "name": "Курс евро к доллару",
        "name_en": "EUR/USD exchange rate",
        "unit": "USD",
        "frequency": "daily",
        "source": "Европейский центральный банк",
        "source_url": (
            "https://www.ecb.europa.eu/stats/policy_and_exchange_rates/"
            "euro_reference_exchange_rates/html/index.en.html"
        ),
        "description": (
            "Справочный курс евро к доллару США: сколько долларов стоит один евро. "
            "Европейский центральный банк публикует его каждый рабочий день "
            "еврозоны около 16:00 по центральноевропейскому времени. Это "
            "справочная величина, а не курс сделки."
        ),
        "methodology": (
            "Ежедневный справочный курс ЕЦБ, доллары США за один евро. Каждая "
            "точка — рабочий день системы TARGET; в выходные и праздники "
            "еврозоны точек нет. На карточке доступны дневное значение и "
            "средние по неделе, месяцу, кварталу и году из того же ряда; "
            "прогноз не строится."
        ),
        "parser_type": "ecb_fx",
        "model_config_json": {
            ...
            "forecast_transform": "absolute",
            "ecb_currency": "USD",
            "backfill_from": "1999-01-04",
            "validation": {"min": 0.5, "max": 2.5},
        },
        "is_active": True,
        "category": "Валюты",
    },
    {
        "code": "gbp-eur",
        "name": "Курс фунта стерлингов к евро",
        "name_en": "GBP per euro",
        "unit": "GBP",
        "frequency": "daily",
        "source": "Европейский центральный банк",
        "source_url": (
        ...
        "description": (
            "Справочный курс ЕЦБ: сколько фунтов стерлингов стоит один евро. "
            "Ряд нужен, чтобы получить курс фунта к доллару как отношение "
            "двух справочных курсов ЕЦБ."
        ),
        "methodology": (
            "Ежедневный справочный курс ЕЦБ, фунты стерлингов за один евро. "
            "Каждая точка — рабочий день TARGET. Самостоятельная карточка "
            "не публикуется: ряд входит в расчёт курса фунта к доллару."
        ),
        "parser_type": "ecb_fx",
        "model_config_json": {
            "forecast_steps": 0,
    ...
    {
        "code": "cny-eur",
        "name": "Курс юаня к евро",
        "name_en": "CNY per euro",
        "unit": "CNY",
        "frequency": "daily",
        "source": "Европейский центральный банк",
        "source_url": (
        ...
        "description": (
            "Справочный курс ЕЦБ: сколько китайских юаней стоит один евро. "
            "Ряд нужен, чтобы получить курс доллара к юаню как отношение "
            "двух справочных курсов ЕЦБ."
        ),
        "methodology": (
            "Ежедневный справочный курс ЕЦБ, юани за один евро. Каждая точка — "
            "рабочий день TARGET. Самостоятельная карточка не публикуется: "
            "ряд входит в расчёт курса доллара к юаню."
        ),
        "parser_type": "ecb_fx",
        "model_config_json": {
            "forecast_steps": 0,
    ...
    {
        "code": "gbp-usd",
        "name": "Курс фунта стерлингов к доллару",
        "name_en": "GBP/USD exchange rate",
        "unit": "USD",
        "frequency": "daily",
        "source": "Европейский центральный банк",
        "source_url": (
        ...
        "description": (
            "Справочный кросс-курс фунта стерлингов к доллару США: сколько "
            "долларов стоит один фунт. Считается по двум справочным курсам "
            "Европейского центрального банка — евро к доллару и фунта к евро. "
            "Это не официальный курс Банка Англии и не цена сделки."
        ),
        "methodology": (
            "Отношение справочного курса доллара за евро к справочному курсу "
            "фунта за евро за один и тот же рабочий день TARGET. Каждая точка — "
            "доллары за один фунт. На карточке доступны дневное значение и "
            "средние по периодам из того же ряда; прогноз не строится."
        ),
        "parser_type": "derived",
        "model_config_json": {
            "forecast_steps": 0,
            "forecast_transform": "absolute",
            "validation": {"min": 0.8, "max": 2.8},
        },
        "is_active": True,
        "category": "Валюты",
    },
    {
        "code": "usd-cny",
        "name": "Курс доллара к юаню",
        "name_en": "USD/CNY exchange rate",
        "unit": "CNY",
        "frequency": "daily",
        "source": "Европейский центральный банк",
        "source_url": (
        ...
        "description": (
            "Справочный кросс-курс доллара США к китайскому юаню: сколько "
            "юаней стоит один доллар. Считается по двум справочным курсам "
            "Европейского центрального банка — юаня к евро и доллара к евро. "
            "Это не центральный паритет Народного банка Китая и не цена сделки."
        ),
        "methodology": (
            "Отношение справочного курса юаня за евро к справочному курсу "
            "доллара за евро за один и тот же рабочий день TARGET. Каждая "
            "точка — юани за один доллар. На карточке доступны дневное "
            "значение и средние по периодам из того же ряда; прогноз не строится."
        ),
        "parser_type": "derived",
        "model_config_json": {
            "forecast_steps": 0,
    ...
    },
    {
        ...
        "unit": "USD/баррель",
        "frequency": "daily",
        "source": "Управление энергетической информации США (EIA)",
        "source_url": "https://www.eia.gov/dnav/pet/hist/RBRTEd.htm",
        "description": (
            "Спотовая цена нефти марки Europe Brent в долларах США за "
            "баррель по итогам каждого торгового дня. Brent — ключевой "
            "эталон мирового нефтяного рынка; его динамику сопоставляют с "
            "другими сырьевыми бенчмарками, курсами валют и ставками."
        ),
        "methodology": (
            "Официальная дневная спотовая цена Europe Brent Spot Price FOB "
            "в долларах за баррель по данным Управления энергетической "
            "информации США. Каждая точка — значение за календарный день "
            "публикации; выходные и праздники в ряд не входят. На карточке "
            "доступны ежедневное значение и средние по неделе, месяцу, "
            "кварталу и году — последние считаются из того же ряда для "
            "удобства сравнения тренда."
        ),
    ...
    },
    {
        ...
        "description": (
            "Мировая цена меди в долларах США за метрическую тонну — месячная "
            "средняя сводки цен на сырьё Всемирного банка. Медь — ключевой "
            "промышленный металл; её цена считается опережающим индикатором "
            "мировой экономики и сырьевого цикла."
        ),
        "methodology": (
            "Месячная средняя цена меди в долларах за тонну по сводке "
            "цен на сырьевые товары Всемирного банка. Каждая точка — "
            "календарный месяц. На карточке доступны месячное значение и "
            "средние по кварталу и году из того же ряда; прогноз не строится."
        ),
        "parser_type": "world_bank_pink_sheet",
        "model_config_json": {
            "forecast_steps": 0,
            "forecast_transform": "absolute",
            "pink_sheet_column": "Copper",
            "backfill_from": "1960-01-01",
    ...
    },
    {
        ...
        "source_url": "https://www.worldbank.org/en/research/commodity-markets",
        "description": (
            "Мировая цена серебра в долларах США за тройскую унцию — месячная "
            "средняя сводки цен на сырьё Всемирного банка. Серебро сочетает "
            "функции драгоценного металла и промышленного сырья."
        ),
        "methodology": (
            "Месячная средняя цена серебра в долларах за тройскую унцию по "
            "сводке цен на сырьевые товары Всемирного банка. Каждая "
            "точка — календарный месяц. На карточке доступны месячное значение "
            "и средние по кварталу и году; прогноз не строится."
        ),
        "parser_type": "world_bank_pink_sheet",
        "model_config_json": {
            "forecast_steps": 0,
            "forecast_transform": "absolute",
            "pink_sheet_column": "Silver",
            "backfill_from": "1960-01-01",
    ...
    },
    {
        ...
        "description": (
            "Спотовая цена природного газа в долларах США за миллион британских "
            "тепловых единиц по эталонному хабу Henry Hub. Газ — один из "
            "ключевых энергоносителей; его цену сопоставляют с нефтью, запасами "
            "и балансом спроса и предложения."
        ),
        "methodology": (
            "Ежедневная спотовая цена Henry Hub по данным Управления "
            "энергетической информации США: каждая точка — торговый день. "
            "На карточке доступны ежедневное значение и средние по неделе, "
            "месяцу, кварталу и году из того же ряда; прогноз не строится."
        ),
        "parser_type": "fred_csv",
        "model_config_json": {
            "forecast_steps": 0,
    ...
    },
    {
        "code": "wheat",
        "name": "Пшеница",
        "name_en": "Wheat (US HRW)",
        "unit": "USD/т",
        "frequency": "monthly",
        "source": "Всемирный банк",
        "source_url": "https://www.worldbank.org/en/research/commodity-markets",
        "description": (
            "Мировая цена пшеницы твёрдых красных сортов США (HRW) в долларах "
            "за метрическую тонну — месячная средняя сводки цен на сырьё "
            "Всемирного банка. Пшеница — базовая продовольственная культура; "
            "её цена отражает мировой баланс урожая, запасов и спроса."
        ),
        "methodology": (
            "Месячная средняя цена пшеницы US HRW в долларах за тонну по сводке "
            "цен на сырьевые товары Всемирного банка. Каждая точка — "
            "календарный месяц. На карточке доступны месячное значение и "
            "средние по кварталу и году; прогноз не строится."
        ),
        "parser_type": "world_bank_pink_sheet",
        "model_config_json": {
            "forecast_steps": 0,
            "forecast_transform": "absolute",
            "pink_sheet_column": "Wheat, US HRW",
            "backfill_from": "1960-01-01",
            "validation": {"min": 30, "max": 2000},
        },
        "is_active": True,
        "category": "Товарные рынки",
    },
    {
        ...
        "source_url": "https://www.worldbank.org/en/research/commodity-markets",
        "description": (
            "Мировая цена соевых бобов в долларах США за метрическую тонну — "
            "месячная средняя сводки цен на сырьё Всемирного банка. Соя — "
            "ключевая сельскохозяйственная культура, индикатор спроса на корма "
            "и растительные масла."
        ),
        "methodology": (
            "Месячная средняя цена соевых бобов в долларах за тонну по сводке "
            "цен на сырьевые товары Всемирного банка. Каждая точка — "
            "календарный месяц. На карточке доступны месячное значение и "
            "средние по кварталу и году; прогноз не строится."
        ),
    ...
    },
    {
        "code": "coal",
        "name": "Уголь",
        "name_en": "Coal (Australia, Newcastle)",
        "unit": "USD/т",
        "frequency": "monthly",
        "source": "Всемирный банк",
        "source_url": "https://www.worldbank.org/en/research/commodity-markets",
        "description": (
            "Мировая цена энергетического угля в долларах США за метрическую "
            "тонну по австралийскому бенчмарку Ньюкасл — месячная средняя "
            "сводки цен на сырьё Всемирного банка. Уголь — один из ключевых "
            "энергоносителей в мировой торговле сырьём."
        ),
        "methodology": (
            "Месячная средняя цена австралийского энергетического угля "
            "(Ньюкасл) в долларах за тонну по сводке цен на сырьевые товары "
            "Всемирного банка. Каждая точка — календарный месяц. "
            "На карточке доступны месячное значение и средние по кварталу и "
            "году; прогноз не строится."
        ),
        "parser_type": "world_bank_pink_sheet",
        "model_config_json": {
            "forecast_steps": 0,
    ...
    },
    {
        ...
        "description": (
            "Карточка снята с витрины: свободного официального ряда мировой "
            "цены горячекатаного проката без коммерческой лицензии биржи нет."
        ),
        "methodology": (
            "Ряд деактивирован. Лондонская биржа металлов и Чикагская биржа "
            "не отдают исторические цены для свободного распространения; "
            "в сводке цен Всемирного банка отдельного ряда горячекатаного "
            "проката нет."
        ),
    ...
    # Годовые оценки МВФ WEO по России — карточки каталога (категория «ВВП» /
    # «Государственные финансы»). Тот же ряд читает overlay рейтинга стран.
    # Прогноз платформы не строится: в ряд входят оценки выпуска фонда.
    {
        "code": "weo-gdp-usd",
        "name": "ВВП в текущих долларах США",
        "name_en": "GDP in current US dollars",
        "unit": "млрд $",
        "frequency": "annual",
        "source": "Международный валютный фонд",
        "source_url": "https://www.imf.org/en/Publications/WEO",
        "description": (
            "Годовая оценка валового внутреннего продукта России в текущих "
            "ценах, млрд долларов США, из публикации «Перспективы развития "
            "мировой экономики» Международного валютного фонда. Это не ряд "
            "Росстата в рублях и не пересчёт национальной валюты по курсу "
            "Банка России."
        ),
        "methodology": (
            "Годовая оценка валового внутреннего продукта в текущих долларах "
            "США. Source: International Monetary Fund, World Economic Outlook. "
            "В ряд входят опубликованные значения и оценки выпуска фонда за "
            "закрытые календарные годы; отдельный прогноз платформы на "
            "карточке не строится."
        ),
        "parser_type": "imf_weo",
        "model_config_json": {
            "weo_code": "NGDPD",
    ...
    {
        "code": "weo-gdp-per-capita-usd",
        "name": "ВВП на душу населения в текущих долларах США",
        "name_en": "GDP per capita in current US dollars",
        "unit": "$ на человека",
        "frequency": "annual",
        "source": "Международный валютный фонд",
        "source_url": "https://www.imf.org/en/Publications/WEO",
        "description": (
            "Годовая оценка валового внутреннего продукта России на душу "
            "населения в текущих долларах США из публикации «Перспективы "
            "развития мировой экономики» Международного валютного фонда. "
            "Это не ряд Росстата и не пересчёт рублей по курсу Банка России."
        ),
        "methodology": (
            "Годовая оценка валового внутреннего продукта на душу населения "
            "в текущих долларах США. Source: International Monetary Fund, "
            "World Economic Outlook. В ряд входят опубликованные значения и "
            "оценки выпуска фонда за закрытые календарные годы; отдельный "
            "прогноз платформы на карточке не строится."
        ),
        "parser_type": "imf_weo",
        "model_config_json": {
            "weo_code": "NGDPDPC",
    ...
    {
        "code": "weo-budget-balance-gdp",
        "name": "Баланс бюджета сектора государственного управления",
        ...
        "source_url": "https://www.imf.org/en/Publications/WEO",
        "description": (
            "Годовая оценка баланса бюджета сектора государственного "
            "управления России в процентах от валового внутреннего продукта "
            "из публикации «Перспективы развития мировой экономики» "
            "Международного валютного фонда. Положительное значение — "
            "профицит, отрицательное — дефицит. Это не кассовое исполнение "
            "бюджета по данным Минфина России: охват сектора и бюджетная "
            "классификация различаются."
        ),
        "methodology": (
        ...
        "is_active": True,
        "category": "Государственные финансы",
    },
    {
        "code": "weo-government-debt-gdp",
        "name": "Государственный долг сектора государственного управления",
        ...
        "source_url": "https://www.imf.org/en/Publications/WEO",
        "description": (
            "Годовая оценка валового долга сектора государственного управления "
            "России в процентах от валового внутреннего продукта из публикации "
            "«Перспективы развития мировой экономики» Международного валютного "
            "фонда. Это не внешний долг по данным Банка России и не кассовые "
            "показатели федерального бюджета Минфина России: охват сектора и "
            "бюджетная классификация различаются."
        ),
        "methodology": (
...
async def _retire_orphan_siblings(db) -> int:
    """Гасит sibling-ряды, которых нет в текущей карте семей.

    Перевод ряда на другую частоту (медь и зерно с дневных котировок на
    месячную сводку) оставляет в БД прежних siblings: они не пересчитываются,
    но остаются активными и отдают старую единицу («USD/фунт» у меди, ставшей
    рядом в долларах за тонну). Bespoke-семьи (ИПЦ, цены производителей) в
    generic-карте не значатся — их базы нет в FAMILIES, поэтому они не
    затрагиваются. 301 на живую карточку даёт `legacy_redirects`.
    """
    from app.data.legacy_redirects import _SIBLING_SUFFIXES
    from app.data.view_model_families import FAMILIES

    known = {mode.code for fam in FAMILIES for mode in fam.modes}
    generic_bases = {fam.base for fam in FAMILIES}
    active = set((await db.execute(
        select(Indicator.code).where(Indicator.is_active.is_(True))
    )).scalars().all())
    ...
    for code in active:
        ...
        for suffix in _SIBLING_SUFFIXES:
            ...
            if base in generic_bases or base not in active:
                orphans.append(code)
            break

    for code in orphans:
        await db.execute(
            update(Indicator).where(Indicator.code == code).values(is_active=False)
        )
    return len(orphans)
...
async def _seed_full():
    async with async_session() as db:
        ...
        retired = await _retire_orphan_siblings(db)
        await db.commit()
        print(
