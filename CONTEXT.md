...
5. **Bind-cookie** `fe_bind` = HMAC(IPv4 /24 или IPv6 /48, UTC-день). Кука чужого префикса → 403 на API, HTML отдаём и перевыпускаем куку (смена соты). Поисковики по UA не режутся; приватный IP — skip. Гео `RUSTATS_SCRAPE_BLOCK_COUNTRIES` — аварийный рычаг (пусто = выкл).
