...
| [`docs/adr/0001`](docs/adr/0001-derived-indicators-engine-shape.md) | Engine shape: 798 derived через `DERIVED_SPECS` (41 ручных + 757 generic) + 27 чистых ops |
...
Хранится в таблице `Indicator`. **Текущее количество (2026-07-08):** 907 рядов в seed; точное число — в `seed_data.py` и `/api/v1/system/status`. Из них 109 source-индикаторов (через 30 парсер-типов) и 798 derived (через `DERIVED_SPECS`: 41 ручных + 757 сгенерированных generic view-mode-семьями, см. `view_model_families.py`).
...
Реестр спецификаций (`calculation_engine.DERIVED_SPECS`) — **798 entries** (41 ручных + 757 из `view_model_families.iter_derived_specs()`). Ручное ядро:
