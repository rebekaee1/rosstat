...
5. **Гео-блок** `RUSTATS_SCRAPE_BLOCK_COUNTRIES=SG` (middleware, поисковики по UA не режутся, `/health` не режется). Пусто = выкл. Ротация SG-IP иначе не банится.
