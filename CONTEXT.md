...
5. **Bind-cookie** `fe_bind` = HMAC(IPv4 /24 или IPv6 /48, UTC-день). Кука чужого префикса → 403 на API данных, HTML отдаём и перевыпускаем куку (смена соты). `/api/v1/auth/*` и `/api/auth/*` не режем: `/login` и `/register` — статический SPA, куку не ставят; callback OAuth приходит с другого /24 после VK/Яндекса. Поисковики по UA не режутся; приватный IP — skip. Гео `RUSTATS_SCRAPE_BLOCK_COUNTRIES` — аварийный рычаг (пусто = выкл).
...
9. **HTML не прячем заглушкой.** Человек без `fe_bind` сразу получает страницу, куку ставим на этом ответе. Тёмные JS-ворота с выдачи теряли клики (2026-09-04/05). Флаг `RUSTATS_SCRAPE_CHALLENGE_ENABLED` режет только API без куки. Гидра снова видит SSR с первого хита — следующий слой: rate-limit / fail2ban, не пустой экран и не 403 VPN.
