...
**Last updated:** 2026-09-20 (локальный пакет `fix/history-access-reliability`; статус и приёмка — [backlog](docs/backlog.md#history-access-reliability-2026-09-20), регламент выкладки — [workflow](docs/workflow.md#прод-деплой)).
...
- **История и индекс:** `index_policy.py` больше не назначает `noindex` по возрасту URL. В sitemap сняты календарные отсечки региональных годов, мировых годов и месячных страниц РФ; данные, listing и canonical-гейты остаются. Для годовых страниц РФ минимум точек: quarterly — 4, annual — 1, monthly и более частые — 6. Мировые годы в sitemap по-прежнему ограничены curated dataset ID, активными странами и listed-рядами: это не открытие всех мировых страниц. `months-N` и потоковая сборка статических sitemap сохраняют всю допустимую историю без накопления полного реестра в памяти. Длинный хвост sitemap кладётся в IndexNow дневной порцией (~30k URL на хост, сначала год ≥ 2018 и спрос Вебмастера); drain раз в 10 минут шлёт батчи и останавливается на 429.
