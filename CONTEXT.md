...
**Инвариант (ADR-0002):** *derived[t] всегда выводимо из текущего state source-рядов*. CalculationEngine при каждом непустом ETL прогоне полностью пересчитывает все 23 derived-ряда от первой до последней точки (idempotent — `bulk_upsert` записывает только реально изменившиеся значения). Не «инкрементальный накопительный снимок», а чистая функция source. Если source ревизуется задним числом, derived перетягиваются автоматически.
