...
| [`docs/adr/0001`](docs/adr/0001-derived-indicators-engine-shape.md) | Engine shape: 823 derived через `DERIVED_SPECS` (44 ручных + 779 generic) + 28 чистых ops |
...
Хранится в таблице `Indicator`. **Текущее количество (2026-08-27):** 940 рядов в seed; точное число — в `seed_data.py` и `/api/v1/system/status`. Из них 117 source-индикаторов (через 34 парсер-типа) и 823 derived (через `DERIVED_SPECS`: 44 ручных + 779 сгенерированных generic view-mode-семьями, см. `view_model_families.py`).
...
Реестр спецификаций (`calculation_engine.DERIVED_SPECS`) — **823 entries** (44 ручных + 779 из `view_model_families.iter_derived_specs()`). Ручное ядро:
...
**Recovery-рецепт (если схема обогнала код):** НЕ рестартовать backend по кругу; (а) свежий `pg-backup.sh`; (б) `docker compose stop backend`; (в) `alembic downgrade <цель>` контейнером НОВОГО образа (он знает цепочку): `docker compose run --rm --no-deps backend alembic downgrade 20260713_partner_revenue` (голову смотреть в репо-коммите, с которого катались); (г) `git checkout <старый SHA>`; (д) `docker compose up -d`; (е) smoke: сайт 200, API 200, `alembic_version` = ожидаемая, `/world*` в sitemap отсутствует. Мировые миграции downgrade безопасны только при пустых таблицах — проверять `SELECT count(*)` ДО downgrade, при данных — стоп и ручное решение.
