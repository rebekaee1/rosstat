...
Лимит backend на 4 ГБ хосте остаётся 1 ГиБ (1536M — после апгрейда RAM);
ClickHouse 448M.
...
UA-фильтр и 429-jail это не ловят.

1. **nginx UA-фильтр** (`$bad_bot` → 403): `research`, `scrapy`, `aiohttp`, `okhttp`, `headlesschrome`, пустой UA.
2. **Ханипот** `/russia/util/links-exchange` (футер SSR) и `/__honeypot__/trap` (в sitemap-core, `robots Disallow`, recrawl skip). Hit = 403 + jail `honeytrap` 48ч.
3. **Rate-limit** `ssrstrict` 2 r/s на региональное семейство; `ssr` 5 r/s; `limit_conn 8`.
4. **fail2ban**: `nginx-429` 30×429/10м; **`nginx-volume` 80×200 GET/10м** (стелс без 429); `honeytrap`; `recidive`.
5. **Гео-блок** `RUSTATS_SCRAPE_BLOCK_COUNTRIES=SG,PL` (middleware, поисковики по UA не режутся, `/health` не режется). Пусто = выкл. Ротация SG/PL-IP иначе не банится. Compose обязан прокидывать переменную — иначе контейнер остаётся на дефолте образа.

Поисковики в `$ssr_limit_key` с пустым ключом. Проверка: `curl -A "Mozilla/5.0 research/1.0" -I https://forecasteconomy.com/` → 403; fail2ban `nginx-volume` после `deploy/fail2ban-install.sh` на хосте.
...
4. **Обновление на SPA-навигации** — `renderFloorAd({refresh:true})` при смене `pathname`: `destroy` прежнего блока + новый `render` с инкрементом `pageNumber` (официальный рецепт РСЯ для динамического контента). Кулдаун `REFRESH_COOLDOWN_MS = 60_000` — чтение 3–4 карточек даёт новое объявление, пролистывание меню не мигает. Раньше `window.__rsyFloorAdRendered` жёстко блокировал повторный render → одно объявление на весь визит.
