...
5. **Bind-cookie** `fe_bind` = HMAC(IPv4 /24 или IPv6 /48, UTC-день). HTML и API 2xx ставят куку. API с кукой чужого префикса → 403 **без** перевыдачи (ретрай не легитимирует). Нет куки — пропускаем и ставим (первый заход, SPA). Поисковики и соцкраулеры по UA не режутся; приватный IP — skip. Гео `RUSTATS_SCRAPE_BLOCK_COUNTRIES` — аварийный рычаг (пусто = выкл).
